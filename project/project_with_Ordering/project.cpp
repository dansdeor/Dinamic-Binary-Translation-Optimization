/*
	Project made by On Fabian and Dan Sdeor
*/

#include "pin.H"
#include "profile.cpp"
#include "rtn-translation-inline.cpp"
#include <assert.h>
#include <errno.h>
#include <fcntl.h>
#include <fstream>
...
#define PROFILING_FILE_NAME "count.csv"
...
void construct_profile_map(ifstream& profiling_file)
{
    string line;
...
INT32 Usage()
{
    cerr << "This tool implements project" << endl;
    cerr << "-prof: run profile and print out routine count information into the file count.csv" << endl;
    cerr << "-opt: run in probe mode make Inline and Code Rordering" << endl;
    return -1;
}
...
int main(int argc, char* argv[])
{
    ...
    if (prof_knob) {
		cout << "Enter - PROF" << endl;
        main_collect_profile(argc, argv);
    } else if (opt_knob) {
		cout << "Enter - OPT" << endl;
        ifstream profiling_file(PROFILING_FILE_NAME);
